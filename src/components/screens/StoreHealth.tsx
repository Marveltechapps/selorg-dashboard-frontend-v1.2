...
const HEALTH_TABS = ['readiness', 'equipment', 'safety'] as const;

export function StoreHealth() {
  const { activeTab, changeTab: setActiveTab } = useScreenTab(HEALTH_TABS, 'readiness');
  ...
  return (
      ...
      actions={
        <div className="flex gap-3 items-center flex-wrap">
          <MetricCard
            variant="inline"
            label="Network"
            value={summary?.network_status || 'Stable'}
            icon={Wifi}
            accent="success"
          ...
          <MetricCard
            variant="inline"
            label="Readiness"
            value={summary?.readiness_status || 'Not Started'}
            icon={ClipboardList}
            accent={
              summary?.readiness_status === 'Ready'
                ? 'success'
                : summary?.readiness_status === 'Partially Ready'
                  ? 'warning'
                  : 'default'
            }
            loading={loadingSummary}
          ...
          <MetricCard
            variant="inline"
            label="Open Issues"
            value={summary ? `${summary.open_issues_count ?? 0} Pending` : '0 Pending'}
            icon={TriangleAlert}
            accent="danger"
            loading={loadingSummary}
          />
    ...
    >
      <DarkstoreTabBar
        ...
        tabs={[
          { id: 'readiness', label: 'Store Readiness', icon: ClipboardList },
          { id: 'equipment', label: 'Equipment Monitoring', icon: Smartphone },
          { id: 'safety', label: 'Safety & Incident Log', icon: ShieldAlert },
        ]}
      />
    ...
    <div className="grid grid-cols-12 gap-6 h-[600px]">
       ...
       <div className="col-span-12 md:col-span-3 space-y-4">
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
             <h3 className="font-bold text-slate-900 mb-4 text-sm uppercase tracking-wider">Checklists</h3>
             <div className="space-y-1">
                <button 
                  ...
                  className={cn(
                    ...
                    activeChecklist === 'opening' ? "bg-blue-50 text-blue-600" : "hover:bg-slate-100 text-slate-600"
                  )}
                >
                ...
                <button 
                   ...
                   className={cn(
                    "w-full flex items-center justify-between p-2 rounded-lg font-bold text-sm transition-colors",
                    activeChecklist === 'closing' ? "bg-blue-50 text-blue-600" : "hover:bg-slate-100 text-slate-600"
                  )}
                >
                ...
                <button 
                   ...
                   className={cn(
                    "w-full flex items-center justify-between p-2 rounded-lg font-bold text-sm transition-colors",
                    activeChecklist === 'hygiene' ? "bg-blue-50 text-blue-600" : "hover:bg-slate-100 text-slate-600"
                  )}
                >
       ...
       <div className="col-span-12 md:col-span-9 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
          <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
             ...
             <div className="text-sm text-slate-500 font-medium">
                Progress: <span className="text-blue-600 font-bold">
                   ...
                   {currentChecklist.items.map((item: any, i: number) => {
                      ...
                      return (
                         <div key={i} className={cn(
                            ...
                            isChecked ? "bg-emerald-50 border-emerald-300" : "bg-white border-slate-200"
                         )}>
                            <div className="flex items-center gap-4">
                               ...
                               <div className={cn("font-medium", isChecked ? "text-emerald-800" : "text-slate-900")}>
                                  {item.task}
                               </div>
                            </div>
             ...
             <button 
                ...
                className={cn(
                  ...
                  currentChecklist?.status === 'completed' 
                    ...
                    : "bg-blue-600 text-white hover:bg-blue-500 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                )}
             >
    ...
    <div className="space-y-6">
       {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
             <MetricCard
               ...
               value={equipment?.summary?.handheld_devices ? `${equipment.summary.handheld_devices.active}/${equipment.summary.handheld_devices.total}` : '...'}
               icon={Smartphone}
               accent={equipment?.summary?.handheld_devices?.offline > 0 ? 'warning' : 'success'}
               footer={<p className="text-xs text-slate-500">Active</p>}
             />
             <MetricCard
               label="Barcode Scanners"
               value={equipment?.summary?.scanners ? `${equipment.summary.scanners.online}/${equipment.summary.scanners.total}` : '...'}
               icon={ScanLine}
               accent="success"
               footer={<p className="text-xs text-slate-500">Online</p>}
             />
             <MetricCard
               label="Network Status"
               value={equipment?.summary?.network?.signal_strength ? `${equipment.summary.network.signal_strength}%` : '...'}
               icon={Wifi}
               accent={equipment?.summary?.network?.status === 'Stable' ? 'purple' : 'warning'}
               footer={<p className="text-xs text-slate-500">Signal Strength</p>}
             />
             <MetricCard
               label="UPS / Power"
               value={equipment?.summary?.power?.battery_level ? `${equipment.summary.power.battery_level}%` : '...'}
               icon={Zap}
               accent="warning"
               footer={<p className="text-xs text-slate-500">Battery Level</p>}
             />
          </div>
       )}
       ...
       <div className="grid grid-cols-12 gap-6 h-[500px]">
          ...
          <div className="col-span-12 md:col-span-8 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
             <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                <h3 className="font-bold text-slate-900">Device Status Log</h3>
                <div className="flex gap-2">
                   <select className="px-3 py-1.5 border border-slate-200 rounded text-xs font-bold text-slate-600">
                      ...
                      {loading ? (
                         ...
                         equipment.devices.map((dev: any, i: number) => (
                            <tr key={i} className="hover:bg-slate-50">
                               <td className="px-4 py-3 font-mono text-xs text-slate-600">{dev.device_id}</td>
                               <td className="px-4 py-3 font-medium text-slate-900">{dev.device_type}</td>
                               ...
                               <td className="px-4 py-3">
                                  <div className="flex items-center gap-2">
                                     <Battery size={14} className={cn((dev.battery_level || 0) < 20 ? "text-red-500" : "text-emerald-600")} />
                                     <span className={cn("text-xs font-bold", (dev.battery_level || 0) < 20 ? "text-red-500" : "text-slate-900")}>
                                        {dev.battery_level || 0}%
                                     </span>
                                  </div>
                               </td>
                               <td className="px-4 py-3">
                                  <div className="flex items-center gap-2">
                                     ...
                                     <span className="text-xs text-slate-600">{dev.signal_strength || 'N/A'}</span>
                                  </div>
                               </td>
                               <td className="px-4 py-3">
                                  <StatusBadge variant="workflow" status={dev.status} />
                               </td>
                            </tr>
                         ))
                      )}
          ...
          <div className="col-span-12 md:col-span-4 flex flex-col gap-6">
             <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex-1">
                <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                   <Wifi size={18} className="text-blue-600" /> Connectivity
                </h3>
                <div className="space-y-4">
                   <div className="p-3 bg-sky-50 border border-sky-200 rounded-lg">
                      <div className="text-xs font-bold text-sky-600 uppercase mb-1">Main Uplink</div>
                      <div className="flex justify-between items-end">
                         <span className="text-lg font-bold text-sky-700">Online</span>
                         <span className="text-xs text-sky-700">Latency: 12ms</span>
                      </div>
                   </div>
                   <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg">
                      <div className="text-xs font-bold text-slate-600 uppercase mb-1">Backup 4G</div>
                      <div className="flex justify-between items-end">
                         <span className="text-lg font-bold text-slate-600">Standby</span>
                         <span className="text-xs text-slate-400">Signal: Good</span>
                      </div>
                   </div>
                </div>
                
                <h3 className="font-bold text-slate-900 mt-6 mb-4 flex items-center gap-2">
                   <Zap size={18} className="text-amber-500" /> Power Backup
                </h3>
                <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                    <div className="text-xs font-bold text-amber-600 uppercase mb-1">Main UPS</div>
                    <div className="flex justify-between items-end">
                       <span className="text-lg font-bold text-amber-600">100% Charged</span>
                       <span className="text-xs text-amber-600">Runtime: 4h 20m</span>
                    </div>
                 </div>
             </div>
          </div>
    ...
    <div className="grid grid-cols-12 gap-6 h-[600px]">
       ...
       <div className="col-span-12 md:col-span-4 space-y-6">
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
             <h3 className="font-bold text-slate-900 mb-4">Safety Overview</h3>
             {loading ? (
                ...
                <>
                   <div className="grid grid-cols-2 gap-4">
                      <div className="p-3 bg-red-50 rounded-lg border border-red-200">
                         ...
                         <div className="text-xs font-bold text-red-800 uppercase">Accidents (Week)</div>
                      </div>
                      <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-300">
                         <div className="text-2xl font-bold text-emerald-600">
                            {summary?.days_safe || 0}
                         </div>
                         <div className="text-xs font-bold text-emerald-800 uppercase">Days Safe</div>
                      </div>
                   </div>
                   ...
                   <div className="mt-6 space-y-3">
                      <div className="space-y-2">
                        <div className="flex items-center justify-between p-3 border border-slate-200 rounded-lg bg-slate-50">
                           <div className="flex flex-col">
                             <span className="text-sm font-medium text-slate-600">Safety Audits</span>
                           ...
                           <button 
                             ...
                             className={cn(
                               ...
                               showAuditsHistory ? "bg-blue-600 text-white" : "text-slate-500 hover:bg-slate-100"
                             )}
                        ...
                        {showAuditsHistory && (
                          <div className="p-3 border border-slate-200 rounded-lg bg-white overflow-y-auto max-h-[200px]">
                            <h4 className="text-[10px] font-bold text-slate-500 uppercase mb-2">Audit History</h4>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between p-3 border border-slate-200 rounded-lg bg-slate-50">
                           <div className="flex flex-col">
                             <span className="text-sm font-medium text-slate-600">Hazard Reports</span>
                           ...
                           <button 
                             ...
                             className={cn(
                               "p-2 rounded-lg transition-colors",
                               showHazardsHistory ? "bg-blue-600 text-white" : "text-slate-500 hover:bg-slate-100"
                             )}
                        ...
                        {showHazardsHistory && (
                          <div className="p-3 border border-slate-200 rounded-lg bg-white overflow-y-auto max-h-[200px]">
                            <h4 className="text-[10px] font-bold text-slate-500 uppercase mb-2">Hazard History</h4>
                ...
                {incidents.filter((inc: any) => inc.status === 'Open' && inc.type === 'hazard').slice(0, 1).map((hazard: any) => (
                   <div key={hazard.incident_id}>
                      ...
                      <button 
                         ...
                         className="w-full py-2 bg-orange-700 text-white text-xs font-bold rounded hover:bg-orange-900 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
       ...
       <div className="col-span-12 md:col-span-8 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
          <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
             <h3 className="font-bold text-slate-900">Incident & Hazard Log</h3>
             
             <Dialog open={isReportModalOpen} onOpenChange={setIsReportModalOpen}>
                <DialogTrigger asChild>
                  <button className="px-3 py-1.5 bg-red-500 text-white rounded text-xs font-bold hover:bg-red-600 flex items-center gap-2">
                  ...
                  <form onSubmit={handleReportIncident} className="space-y-4 py-4">
                    ...
                    <DialogFooter>
                      <button 
                        ...
                        className="w-full py-2 bg-red-500 text-white font-bold rounded-lg hover:bg-red-600 disabled:opacity-50"
                      >
                   ...
                   <tbody className="divide-y divide-slate-100">
                      {incidents.map((incident: any, i: number) => (
                         <tr key={i} className="hover:bg-slate-50">
                            ...
                            <td className="px-4 py-3 text-slate-900 max-w-[200px] truncate">{incident.description}</td>
                            <td className="px-4 py-3 text-slate-600 text-xs">{incident.reported_by}</td>
                            <td className="px-4 py-3">
                               <StatusBadge variant="workflow" status={incident.status} />
                            </td>
                            <td className="px-4 py-3">
                               {incident.status === 'Open' && (
                                  <button 
                                     ...
                                     className="text-blue-600 font-bold text-xs hover:underline disabled:opacity-50 disabled:cursor-not-allowed"
                                  >
